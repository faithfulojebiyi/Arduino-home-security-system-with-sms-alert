...
int speakerPin = 35; 
//int relay1 = 3; // 
int relay2 = 4; // connected to camera
int relay3 = 5; // connected to 12V Blue LED strip
int relay4 = 6; // 
...
void setup(){
  ...
  digitalWrite(relay2, HIGH); //  camera, 5V external DC supply
  digitalWrite(relay3, HIGH); // 12V Blue LED lighting
...
void displayCodeEntryScreen()    // Dispalying start screen for users to enter PIN
{
  ...
  lcd.setCursor(0,3);
  lcd.print("Ojebiyi Faithful Oludunsin");
}
